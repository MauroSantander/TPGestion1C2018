...
RETURN(SELECT 
(CASE 
WHEN ht.nombre IS NULL THEN  LTRIM(RTRIM(ht.ciudad)) +'-'+ 
LTRIM(RTRIM(ht.calle)) + '-' + LTRIM(RTRIM(CONVERT(VARCHAR(255),ht.nroCalle)))
WHEN ht.nombre IS NOT NULL THEN ht.nombre
END) AS Hotel,
hb.piso AS Piso,
hb.numero AS Numero,
hb.tipo AS Tipo,
...
FROM [PISOS_PICADOS].Habitacion AS hb JOIN [PISOS_PICADOS].Hotel AS ht ON hb.idHotel = ht.idHotel )
GO
